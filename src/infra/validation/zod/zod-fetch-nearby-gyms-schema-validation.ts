...
const fetchNearbyGymsQuerySchema = z.object({
  userLatitude: z.string().transform((val) => (val ? parseFloat(val) : 0)), // Transforme em número e substitua undefined por 0
  userLongitude: z.string().transform((val) => (val ? parseFloat(val) : 0)), // Transforme em número e substitua undefined por 0
})
